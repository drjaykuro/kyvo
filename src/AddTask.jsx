...
const RECURRING_OPTIONS = [
  { key: null, label: 'None', icon: Ban },
  { key: 'daily', label: 'Daily', icon: RotateCw },
...
function AddTask({ userId, onBack, onDone }) {
  ...
  async function handleSubmit() {
    ...
    if (subtasks.length > 0) {
      const rows = subtasks.map((subName) => ({
        task_id: newTask.id,
        name: subName,
        done: false,
      }))
  ...
  return (
    <div className="add-task-screen">
      ...
      <p className="add-task-subheading">
        Build something <span className="highlight">important</span> today
      </p>
      ...
      <div className="icon-grid">
        {CATEGORIES.map(({ key, icon: Icon }) => (
          <button
            key={key}
            type="button"
            className={`icon-choice${category === key ? ' active' : ''}`}
      ...
      <div className="icon-grid">
        {DIFFICULTIES.map(({ key, flames }) => (
          <button
            key={key}
            type="button"
            className={`icon-choice wide${difficulty === key ? ' active' : ''}`}
      ...
      <div className="icon-grid">
        {RECURRING_OPTIONS.map(({ key, icon: Icon, label }) => (
          <button
            key={label}
            type="button"
            className={`icon-choice${recurring === key ? ' active' : ''}`}
      ...
      <p className="selection-caption">
        Repeat: {RECURRING_OPTIONS.find((r) => r.key === recurring)?.label}
      </p>
      ...
      {previewValid && (
        <div className="preview-card">
          <span
            ...
            style={{ '--block-color': SIZE_COLOR_VAR[previewSize] }}
          >
            {previewSize?.toUpperCase()}
          </span>
          <span className="preview-xp">
            Estimated XP: <strong>+{previewXp}</strong>
          </span>
        </div>
      )}
        ...
        <div className="subtask-chip-list">
          {subtasks.map((s, i) => (
            <span className="subtask-chip" key={i}>
              {s}
              <button onClick={() => removeSubtaskDraft(i)} aria-label="Remove">
                <X size={13} />
              </button>
            </span>
          ))}
      ...
      <div className="add-subtask-row">
        <input
          ...
          onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addSubtaskDraft())}
        />
        <button type="button" className="add-subtask-button" onClick={addSubtaskDraft}>
          Add
        </button>
      </div>

      <button className="add-task-button shine" onClick={handleSubmit} disabled={saving}>
        {saving ? 'Placing block…' : '+ Add Task'}
      </button>
    </div>
  )
}
